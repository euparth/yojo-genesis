 ...
 * connected by faint network lines. Gently repels from the cursor.
 */
export function NatureCanvas({ className = "" }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    function step(t: number) {
      ...
      // connection lines first (the "tech" layer)
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 110) {
            ...
            ctx!.strokeStyle = `rgba(${MOSS}, ${alpha})`;
            ctx!.lineWidth = 0.7;
            ctx!.beginPath();
            ctx!.moveTo(a.x, a.y);
            ctx!.lineTo(b.x, b.y);
      ...
      for (const p of particles) {
        // gentle cursor repulsion — interactive
        ...
        if (p.kind === "leaf") {
          // falling-leaf ellipse
          ctx!.save();
          ctx!.translate(p.x, p.y);
          ctx!.rotate(Math.sin(t / 1400 + p.phase) * 0.8);
          ctx!.fillStyle = `rgba(${MOSS}, 0.35)`;
          ctx!.beginPath();
          ctx!.ellipse(0, 0, p.r * 2.1, p.r * 0.95, 0, 0, Math.PI * 2);
          ctx!.fill();
          ctx!.restore();
        } else {
          // data node
